...
import 'package:agora_ogaboss/pages/PostScreenPage.dart';
...
class _NotificationsPageState extends State<NotificationsPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: header(context, strTittle: "Notifications"),
      body:  Container(
        child: FutureBuilder(
  ...
  retrieveNotifications()async{
    QuerySnapshot querySnapshot = await activityFeedReference.document(currentUser.id).collection("feedItems")
        .orderBy("timestamp",descending: true).limit(60).getDocuments();
