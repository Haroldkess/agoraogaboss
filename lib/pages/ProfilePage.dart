...
import 'package:agora_ogaboss/pages/EditProfilePage.dart';
import 'package:agora_ogaboss/pages/HomePage.dart';
import 'package:agora_ogaboss/widgets/HeaderWidget.dart';
import 'package:agora_ogaboss/widgets/PostTileWidget.dart';
...
class _ProfilePageState extends State<ProfilePage> {
  ...
  {
    DocumentSnapshot documentSnapshot = await followersRefrence.document(widget.userProfileId)
          ...
          child: Column(
            children: <Widget>[
              ...
              Container(
                ...
                child: Text(user.username,style: TextStyle(fontSize: 14.0,color: Colors.white),),
              ),
              Container(
                alignment: Alignment.centerLeft,
  ...
  Column createColums(String tittle, int count) {
    return Column(
  ...
  createButton()
  {
    ...
    if (ownProfile)
    {
      return createButtonTittleAndFunction(
  ...
  controlUnfollowUser()
  {
    ...
    followingRefrence.document(currentOnlineUserId)
        ...
        .document(widget.userProfileId).get()
        .then((document){
      if(document.exists)
      {
        document.reference.delete();
  ...
  createButtonTittleAndFunction({String title, Function performFunction}) {
    return Container(
      padding: EdgeInsets.only( top: 3.0 ),
      child: FlatButton(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
              color: following?  Colors.black: Colors.white70, border: Border.all( color: Colors.grey ), borderRadius: BorderRadius.circular(6.0)),
        ),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: header( context, strTittle: "Profile" ),
        body: ListView(
          children: <Widget>[
            createProfileTopView(),
            Divider(),
